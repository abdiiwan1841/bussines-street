...
//            echo "===";
    ...
    public function SaveRegBerhasil($data) {
       ...
       $data['enc_password'] = $this->strToHex($this->rc4($vcode, $dec));
        $sql = "
            select 0;
            insert into sys_users(username, password,isemailverified)
            values(:email, :enc_password,1)
			;
            insert into sys_userroles(userid, roleid)
            select currval('sys_users_userid_seq') as userid, roleid
            from sys_roles where rolename = 'user'
            ...
            ";	
			 ...
			 echo "<h1>Selamat Registrasi Anda Sukses</h1>";
        //$result = $this->mdb->QueryData('application', $sql, $data, 'record');
       // return $result[0];
    }
    ...
    public function saveRegistration($data){
        ...
        $sql = "
            select 0;
            insert into app_registration(email, full_name, jenkelid, birth_city, birth_date, password, pwd_key)
            values(:email, :full_name, :jenkelid, :birth_city, to_date(:birth_date, 'dd-mm-yyyy'), :password, :pwd_key);
	...
	function doGeekSendEmail($to, $subject, $message){
	$ci = get_instance();
	$ci->load->library('email');
	$config['protocol'] = "smtp";
	$config['smtp_host'] = "ssl://smtp.gmail.com";
	$config['smtp_port'] = "465";
	$config['smtp_user'] = "alimindev762@gmail.com"; 
	$config['smtp_pass'] = "87654321alimin";
	$config['charset'] = "utf-8";
	$config['mailtype'] = "html";
	$config['newline'] = "\r\n";
	$ci->email->initialize($config);
	$ci->email->from('alimindev762@gmail.com', 'Alimin Dev -');
	$ci->email->to($to);
	$this->email->reply_to('alimindev762@gmail.com', 'Email sukses terkirim');
	$ci->email->subject('Registrasi Wikitani');
