...
class Login extends MY_Controller
{
    ...
    function index(){
        ...
        if ($sessData===false){
			...
			$userid=$this->mglobal->getDataUserByUsername($sessData["username"])["userid"];
			$_SESSION['session_login'] = array_merge($sessData,array("userid"=>$userid));
			
			//print_r($_SESSION['session_login']); die();
			if($_GET["return_page_to"]== "Search Engine"){
				//print_r($sessData); die();		
				//$_SESSION['session_login'] = $sessData;
				//echo $_SESSION['username']; die();
				header('Location: '.base_url()."index.php/search_engine?return_page_to=".$_GET["return_page_to"]);		
				
			}else if($_GET["return_page_to"]== "Bussines Plan Form"){
				header('Location: '.base_url()."index.php/search_engine?return_page_to=".$_GET["return_page_to"]);		
			}else{		
				header('Location: '.base_url()."index.php/search_engine/doSearch?textsearch=".$_GET["return_keyword_to"]);	
			}
        }
