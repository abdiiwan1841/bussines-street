...
class search_engine extends MY_Controller
{
    ...
    function index(){
		...
		$data["data_bussines"]=$this->mbussines->getDataBussinesAll();
		//print_r
        $data   = $this->mmenu->getDefaultData();
		$sess_data=$this->mlogin->getSessionData();
		if(isset($sess_data["username"])){
		$global_userid= $this->mglobal->getDataUserByUsername($sess_data["username"])["userid"];
		$data["global_user_id"]=$global_userid;
		$foto_profile= $this->mglobal->getPhotoProfile($global_userid)["photo_name"];
		$dir_name=$this->mglobal->getPhotoProfile($global_userid)["dir_name"];
		...
		$html = $this->load->view("v_searchengine",$data,TRUE);
		
		$data["pagecontent"]=$html;
       	$data["pageid"]="search-engine";
